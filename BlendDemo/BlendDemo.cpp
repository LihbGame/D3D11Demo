...
private:
	...
	ConstBuffers mConstBuf;
	ID3D11SamplerState* m_pAnisotropySampler;
};
...
bool BlendApp::Init()
{
	...
	Shader::InitAll();
	Shader::CreateAllShaders(md3dDevice);
	CreateConstBuffer();
	...
	md3dDevice->CreateSamplerState(&samplerDesc, &m_pAnisotropySampler);
...
void BlendApp::DrawScene()
{
	...
	md3dImmediateContext->VSSetShader(Shader::pVSShader, NULL, 0);
	md3dImmediateContext->PSSetShader(Shader::pPSShader, NULL, 0);
	md3dImmediateContext->VSSetConstantBuffers(0, 1, &m_ConstBuffer);
	md3dImmediateContext->PSSetConstantBuffers(0, 1, &m_ConstBuffer);
	md3dImmediateContext->PSSetSamplers(0, 1, &m_pAnisotropySampler);
	...
	mConstBuf.mDirLights[0] = mDirLights[0];
	mConstBuf.mDirLights[1] = mDirLights[1];
	mConstBuf.mDirLights[2] = mDirLights[2];
	mConstBuf.EyePos = mEyePosW;
	XMStoreFloat4(&mConstBuf.gFogColor,Colors::Silver);
	mConstBuf.gFogRange = 175.0f;
	mConstBuf.gFogStart = 15.0f;
	mConstBuf.gMaterial = mBoxMat;
	XMStoreFloat4x4(&mConstBuf.gTexTransform, XMMatrixIdentity());
	mConstBuf.gWorld = mBoxWorld;
	XMStoreFloat4x4(&mConstBuf.gWorldInvTranspose, worldInvTranspose);
	XMStoreFloat4x4(&mConstBuf.gWorldViewProj, worldViewProj);
	md3dImmediateContext->UpdateSubresource(m_ConstBuffer, 0, NULL, &mConstBuf, 0, 0);
...
void BlendApp::CreateConstBuffer()
{
	static D3D11_BUFFER_DESC desc =
	{
		sizeof(ConstBuffers), //ByteWidth
		D3D11_USAGE_DEFAULT, //Usage
		D3D11_BIND_CONSTANT_BUFFER, //BindFlags
		0, //CPUAccessFlags
		0  //MiscFlags
	};
	md3dDevice->CreateBuffer(&desc, NULL, &m_ConstBuffer);
}
